...
class Patient:
    def __init__(self, id, parameters):
        ...
        # times that develop stroke
        self._times_develop_stroke = 0
        # cost of life time
        self._cost = 0
    ...
    def simulate(self, sim_length):
        ...
        while self._stateMonitor.get_if_alive() and k*self._delta_t < sim_length:
            ...
            if self._stateMonitor.get_current_state() == P.HealthStats.STROKE:
                self._stateMonitor.update(k, P.HealthStats(new_state_index))
                self._times_develop_stroke += 1
                k += 0

            else:
                self._stateMonitor.update(k, P.HealthStats(new_state_index)) # update health state
                #  increment time step
                k += 1
    ...
    def get_stroke_times(self):
        """ stroke times  """
        return self._times_develop_stroke

    def get_cost(self):
        self._cost = Data.STROKE_COST*self._times_develop_stroke
        return self._cost
...
class CohortOutputs:
    def __init__(self, simulated_cohort):
        ...
        self._times_stroke = []        # patients' times to have storke
        ...
        for patient in simulated_cohort.get_patients():
            ...
            stroke = patient.get_stroke_times()
            self._times_stroke.append(stroke)
    ...
    def get_cohort_stroke(self):
        return self._times_stroke